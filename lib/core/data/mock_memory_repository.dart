...
///
/// [clock] exists so tests — goldens especially — can pin every seeded
/// date, greeting bucket and timeline day to one instant. Production
/// passes nothing and gets the wall clock.
class MockMemoryRepository {
  MockMemoryRepository({DateTime Function()? clock})
      : this._(clock ?? DateTime.now);

  MockMemoryRepository._(DateTime Function() clock)
      : _clock = clock,
        _items = _buildSeed(clock()),
        collections = _buildCollections(clock()),
        connections = _buildConnections(clock());
  ...
  SearchOutcome search(String query, {MemoryType? filter}) {
    ...
    for (final item in _items) {
      ...
      if (score == 0) continue;
      final ageDays = _clock().difference(item.date).inDays;
  ...
  MemoryStats stats() => MemoryStats(
        ...
        lastIndexed: _clock().subtract(const Duration(minutes: 2)),
      );

  final List<MemoryCollection> collections;

  static List<MemoryCollection> _buildCollections(DateTime now) => [
    MemoryCollection(
      id: 'voxbridge',
      name: 'VoxBridge',
      itemCount: 24,
      updated: now.subtract(const Duration(hours: 3)),
    ...
    MemoryCollection(
      ...
      name: 'Receipts',
      itemCount: 32,
      updated: now.subtract(const Duration(hours: 8)),
  ...
  static List<ConnectionInfo> _buildConnections(DateTime now) => [
    ConnectionInfo(
      id: 'photos',
      name: 'Photos',
    ...
    ConnectionInfo(
      ...
      lastSync: now.subtract(const Duration(days: 6)),
...
List<MemoryItem> _buildSeed(DateTime now) => [
  MemoryItem(
    id: 'doc-vox-pricing',
