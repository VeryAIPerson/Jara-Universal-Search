...
| ~~D20~~ | ~~Tipli Dart deck → ARB + gen_l10n geçişi~~ | — | **GERİ ALINDI → D20b** |
| D20b | **Tipli deck korunur**, dil başına bir `strings_XX.dart`; ARB yalnız çeviri ajansına *dışa aktarım* formatı olarak üretilir | 20 dilde asıl risk eksik anahtar. ARB'de eksik anahtar sessizce şablon dile düşer ve fark edilmez; tipli deck'te **derleme hatası** olur — 20 dilde parite garantisi ancak böyle verilebilir. Ayrıca çağrı yerleri hiç değişmediği için 18 dil paralel yazılabiliyor. Çoğul kuralları (ru/pl/ar) her dil sınıfında kendi mantığıyla, `errorTitle(JaraError)` gibi switch tabanlı API ARB'de ifade edilemezdi | KİLİTLİ |
...
| D22 | Wear OS store dağıtımı **ayrı `wear` product flavour** ile; tek manifest ikisine birden hizmet edemez | Play'in Wear form faktörü `uses-feature ... required="true"` istiyor, ama bu bayrak telefon build'ini Play'den **tamamen filtreliyor**. Ortak manifest `required="false"` kalır (yan yükleme + geliştirme çalışır, telefon etkilenmez); store için `src/wear/AndroidManifest.xml` override'ı + `-t lib/main_watch.dart` pinleyen flavour eklenir | KİLİTLİ (flavour entegrasyon turunda) |
| D23 | Hintçe'de `कल` belirsizliği **uzun ama net** formla çözülür (`आने वाला कल`/`बीता कल`) | Tarih rozetinde yanlış günü göstermek, iki kelime uzunluktan çok daha pahalı | KİLİTLİ |
| D24 | "Cloud Intelligence" gibi **özellik adları çevrilir**; marka kilidi yalnız ürün adlarını (JARA, Voxbridge, Universal Search) kapsar | Aksi halde 20 dilde Latin harfli özellik adları kullanıcıyı yabancılaştırır; ürün adı ise aile standardıyla kilitli | KİLİTLİ |
